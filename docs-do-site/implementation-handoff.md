...
- Mobile (<768px): Float GSAP era desligado, mas foi corrigido na V3 para ser responsivo e ativo.
...
## GSAP Responsive Motion V3
- Decisão: O Ambient Float deve rodar em todos os viewports (Mobile, Tablet, Desktop), adaptando a amplitude e velocidade em vez de simplesmente desligar.
- Exceção (Acessibilidade): O único trigger de cancelamento absoluto do movimento é prefers-reduced-motion: reduce, gerido nativamente pelo gsap.matchMedia().
- FloatingMotion API: Extensão tipada aceitando { mobile, tablet, desktop } com vetores de rotação, amplitude (x/y), delay e duration isolados por viewport.
- Desktop Parameters: Intactos (V2).
