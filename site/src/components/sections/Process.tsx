...
export function Process() { return <section id="como-funciona" className="section-block bg-background-primary" aria-labelledby="process-heading"><Container><div className="process-intro"><Eyebrow>Como funciona</Eyebrow><h2 id="process-heading" className="type-heading-xl mt-3 max-w-[24ch] text-text-primary">Três etapas para tornar o início mais claro.</h2></div><div className="process-region mt-8">{steps.map(({ icon: Icon, body, ...step }) => <ProcessStep key={step.number} {...step} icon={<Icon aria-hidden="true" size={20} strokeWidth={1.5} />}>{body}</ProcessStep>)}</div></Container></section>; }
